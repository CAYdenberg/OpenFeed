...
import { determineFeedId } from '../helpers';
import {
  constants as newFeedConstants,
  actions as newFeedActions,
} from './newFeed';
import { constants as feedsConstants, actions as feedsActions } from './feeds';
...
function* dbReadySaga(action) {
  ...
  yield all([put(feedsActions.loadFeeds())]);
...
function* addFeedToDBSaga() {
  const { loadState, feed, posts, value } = yield select(
    state => state.newFeed
  );
